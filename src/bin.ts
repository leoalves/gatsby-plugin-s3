...
const getBucketInfo = async (
  ...
  s3: S3
): Promise<{ exists: boolean; region?: string }> => {
  try {
    const { $response } = await s3
      .getBucketLocation({ Bucket: config.bucketName })
      .promise()
    const detectedRegion = guessRegion(
      s3,
      $response.data && $response.data.LocationConstraint
    )
...
const listAllObjects = async (
  ...
  if (response.NextContinuationToken) {
    list.push(
      ...(await listAllObjects(s3, bucketName, response.NextContinuationToken))
    )
  }
...
`)
    ...
    stream.on('data', async ({ path, stats }) => {
      ...
      const key = createSafeS3Key(relative('./public', path))
      const buffer = await readFile(path)
      const tag = `"${createHash('md5')
        .update(buffer)
        .digest('hex')}"`
      const object = objects.find(
        (object: any) => object.Key === key && object.ETag === tag
      )
    ...
    if (config.removeNonexistentObjects) {
      const objectsToRemove = objects
        .map((obj: any) => ({ Key: <string>obj.Key }))
        .filter((obj: any) => obj.Key && !isKeyInUse[obj.Key])
      ...
      for (
        ...
        spinner.text = `Removing objects ${i + 1} to ${i +
    ...
    console.log(chalk`
        ...
        {blue.underline http://${config.bucketName}.s3-website-${region ||
